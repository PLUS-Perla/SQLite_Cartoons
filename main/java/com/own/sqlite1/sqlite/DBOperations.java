...
public final class DBOperations {
    ...
    //Operations of  Cartoon
    public Cursor getCartoons(){
        SQLiteDatabase database = db.getReadableDatabase();
        String sql = String.format("SELECT * FROM %s",
                DBHelper.Tables.CARTOON);
        return  database.rawQuery(sql, null);
    }

    public Cursor getCartoonById(String id){
        SQLiteDatabase database = db.getReadableDatabase();
        String sql = String.format("SELECT * FROM %s" +
                        " WHERE %s=?",
                DBHelper.Tables.CARTOON,
                Contract.Cartoons.ID);
        String[] whereArgs = {id};
        return database.rawQuery(sql, whereArgs);
    }

    public String insertCartoon(Cartoon cartoon){
        SQLiteDatabase database = db.getWritableDatabase();
        String idCartoon = Contract.Cartoons.generateIdCartoon();
        ContentValues values = new ContentValues();
        values.put(Contract.Cartoons.ID, idCartoon);
        values.put(Contract.Cartoons.NAME, cartoon.getName());
        values.put(Contract.Cartoons.TYPE, cartoon.getType());
        values.put(Contract.Cartoons.LANGUAGE, cartoon.getLanguage());
        values.put(Contract.Cartoons.AUTHOR, cartoon.getAuthor());
        values.put(Contract.Cartoons.CAPITULOS, cartoon.getCapitulos());
        values.put(Contract.Cartoons.YEAR, cartoon.getYear());
        database.insertOrThrow(DBHelper.Tables.CARTOON, null, values);
        return idCartoon;
    }
    ...
    public boolean updateCartoon(Cartoon cartoon){
        ...
        values.put(Contract.Cartoons.CAPITULOS, cartoon.getCapitulos());
        values.put(Contract.Cartoons.YEAR, cartoon.getYear());
        String whereClause = String.format("%s=?", Contract.Cartoons.ID);
        String[] whereArgs = {cartoon.getIdCartoon()};
        int result = database.update(DBHelper.Tables.CARTOON,
                values, whereClause, whereArgs);
        return result>0;
    }

    public boolean deleteCartoon(String idCartoon) {
        SQLiteDatabase database = db.getWritableDatabase();
        String whereClause = Contract.Cartoons.ID + "=?";
        String[] whereArgs = {idCartoon};
        int result = database.delete(DBHelper.Tables.CARTOON,
                whereClause, whereArgs);
        return result > 0;
    }
